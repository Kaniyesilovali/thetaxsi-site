...
export function renderBook(ctx) {
  ...
  const textareaCls =
    'rounded-xl border border-line bg-paper px-4 py-3 text-sm text-ink outline-none transition-all focus:border-sea focus:ring-2 focus:ring-sea/20'
  // Seçim kutusu — yuvarlak, ince çizgi, açık zeminli forma uyumlu
  const toggleCardCls = 'rounded-2xl border border-line bg-paper px-4 py-3'
...
<section class="bg-fog py-16 lg:py-24">
  <div class="mx-auto grid max-w-6xl gap-12 px-5 sm:px-8 lg:grid-cols-[1fr_380px]">
    <form id="booking-form" class="grid gap-6 rounded-3xl border border-line bg-paper p-6 sm:p-8" novalidate>
      <h2 class="text-2xl font-semibold tracking-tight text-ink sm:text-3xl">${esc(t.route.heading)}</h2>
      ...
      <div id="bf-hotel-wrap" class="hidden flex-col gap-2">
        ...
        <p id="bf-hotel-error" class="hidden text-sm text-red-600">${esc(f.hotelValidation)}</p>
      </div>
      ...
      <div id="bf-flight-wrap" class="hidden flex-col gap-2">
        ...
        <p id="bf-flight-error" class="hidden text-sm text-red-600">${esc(f.flightValidation)}</p>
      </div>
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <label class="flex cursor-pointer items-center justify-between gap-4 ${toggleCardCls} transition-colors hover:border-sea/40">
          <span>
            <span class="block text-sm font-medium text-ink">${esc(f.roundTripLabel)}</span>
            <span class="mt-0.5 block text-xs text-slate">${esc(t.route.roundTripDesc)}</span>
          </span>
          <input type="checkbox" id="bf-roundtrip" name="roundtrip" class="size-4 accent-sea">
        </label>
        ...
        <label class="flex items-center justify-between gap-4 ${toggleCardCls}">
          <span class="block text-sm font-medium text-ink">${esc(f.childSeatLabel)}</span>
          <select id="bf-childseat" name="childseat" class="h-10 w-16 rounded-lg border border-line bg-paper px-2 text-sm text-ink outline-none transition-all focus:border-sea focus:ring-2 focus:ring-sea/20 [color-scheme:light] [&>option]:bg-white [&>option]:text-ink">
            ${[0, 1, 2, 3, 4].map((n) => `<option value="${n}">${n}</option>`).join('')}
          </select>
        </label>
      </div>

      <h2 class="mt-6 text-2xl font-semibold tracking-tight text-ink sm:text-3xl">${esc(t.details.heading)}</h2>
      <div class="grid gap-6 sm:grid-cols-2">
      ...
      ${field(`${label('bf-notes', f.notesLabel)}<textarea id="bf-notes" name="notes" rows="3" placeholder="${esc(f.notesPlaceholder)}" class="${textareaCls}"></textarea>`)}
    ...
    <aside class="h-fit rounded-3xl border border-line bg-paper p-7 shadow-card lg:sticky lg:top-24">
      <p class="text-sm font-medium text-sea">${esc(t.summary.title)}</p>
      ...
      <div class="mt-8 border-t border-line pt-6">
        <p class="text-[11px] font-medium uppercase tracking-[0.14em] text-slate">${esc(rd.includedTitle)}</p>
